...
- **Приложения — PhotoGIMP: Permission denied при клонировании**: `mktemp -d` выполнялся от root и создавал папку в `/tmp/.private/root/`, недоступную пользователю; заменено на `sudo -u $REAL_USER mktemp -d`; `rm -rf "$TMP"` в конце заменён на `trap ... EXIT` для гарантированной очистки при ошибке
