...
### Добавлено
- **Адаптивная раскладка (GNOME HIG)**: при ширине окна < 550sp переключатель вкладок перемещается из заголовка на нижнюю панель (`Adw.ViewSwitcherBar`); реализовано через `Adw.BreakpointBin` + `Adw.Breakpoint`
